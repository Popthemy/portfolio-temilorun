...
export const Projects: React.FC = () => {
  ...
  useEffect(() => {
    ...
    cards.forEach((card, i) => {
      ...
      gsap.set(card, {
        ...
        display: isMobile && Math.abs(i - currentIndex) > 1 && Math.abs(i - currentIndex) < totalProjects - 1 ? 'none' : 'block'
      });
    });
  ...
  const handlePrev = () => rotateCarousel((currentIndex - 1 + totalProjects) % totalProjects);

  return (
    <section id="projects" className="py-12 md:py-32 overflow-hidden bg-gray-50/50 dark:bg-slate-950/50 transition-colors duration-300">
      <div className="max-w-7xl mx-auto px-6">
        ...
        >
          <div>
            ...
            <p className="text-base md:text-xl text-gray-500 dark:text-gray-400 mt-4 max-w-md">Real solutions delivered with measurable impact and technical precision.</p>
          </div>
          <a href="#" className="mt-6 md:mt-0 text-primary font-bold flex items-center group">
            View all on GitHub <ExternalLink size={20} className="ml-2 group-hover:rotate-45 transition-transform" />
          </a>
        </motion.div>

        <div className="relative h-[420px] md:h-[850px] flex items-center justify-center perspective-[2000px]">
          {/* Navigation Controls */}
          ...
          >
            {PROJECTS.map((project, index) => (
              <div 
                key={project.id} 
                className="project-card-wrapper absolute w-[280px] md:w-[500px]"
                style={{
